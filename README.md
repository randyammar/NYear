...
按理说影响系统程序访问数据库性能最深刻的是业务程序本身，ORM 框架只是一个桥梁工具罢了。</br>
但就是这个工具就会把开发人员带偏，走向万劫不复的深渊，EF就是一个好很的反面教材；</br>
EF本身数据映性能就不好、SQL语句生成速度差，但最不靠谱的也是生成出来的SQL有很多的转换，造成SQL语句复杂、走不上索引，还没办法优化；</br>
导致在数据库里头运行的这些SQL语句简直就是奇慢无比，再加上EF、Linq的语法需要花大力气去学，使用起来的开发效率也十分低下。</br>
